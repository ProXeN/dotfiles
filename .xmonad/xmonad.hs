...
--    , ((super, xK_l), nextScreen)
--    , ((super, xK_k), prevScreen)
--    , ((super .|. shiftMask, xK_l), shiftToNext)
--    , ((super .|. shiftMask, xK_k), shiftToPrev)

    -- Swap the focused window with the next window
    , ((super .|. shiftMask, xK_j), windows W.swapDown)
    , ((super .|. shiftMask, xK_Left), windows W.swapDown)

    -- Swap the focused window with he previous window
...
myManageHook = composeAll . concat $
    ...
    , [className =? "Vivaldi-stable" --> viewShift "2"]
    , [className =? "TelegramDesktop" --> viewShift "4"]
...
defaults = def {
      ...
      -- hooks, layouts
        ...
        layoutHook = gaps [(L,10), (R,10), (U,10), (D,10)] $ spacingRaw False (Border 10 0 10 0) True (Border 0 10 0 10) True $ myLayout,
